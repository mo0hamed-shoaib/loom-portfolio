...
export function MilestoneCard({ milestone }: MilestoneCardProps) {
  ...
  return (
    <MilestoneDetailModal milestone={milestone}>
      <Card className="cursor-pointer border-border/50 hover:border-border hover:shadow-sm transition-all duration-200 h-full p-0 card-hover">
        <CardContent className="p-4 space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div className={`p-2 rounded-lg ${colorClass} hover-scale`}>
              <Icon className="w-4 h-4" />
            </div>
            <div className="text-right">
              <p className="text-xs sm:text-sm 3xl:text-base text-foreground">
          ...
          {/* Title */}
          <div>
            <h3 className="typography-h4 leading-tight line-clamp-2">
              {milestone.title}
            </h3>
          </div>
          ...
          {milestone.description && (
            <p className="font-mono-technical text-xs sm:text-sm 3xl:text-base text-foreground line-clamp-3 leading-relaxed">
              {milestone.description}
            </p>
          )}

          {/* Key achievements preview */}
          {milestone.keyAchievements &&
            milestone.keyAchievements.length > 0 && (
              <div className="pt-1">
                <p className="font-mono-technical text-xs text-foreground">
                  {milestone.keyAchievements[0]}
                  {milestone.keyAchievements.length > 1 &&
                    ` +${milestone.keyAchievements.length - 1} more`}
                </p>
              </div>
            )}
        </CardContent>
