...
const techIcons: Record<string, { light: string; dark?: string }> = {
  ...
  shadcnui: {
    light: "/icons/frontend/shadcn-dark.svg",
    dark: "/icons/frontend/shadcn-light.svg",
  },
...
export function TechIcon({
  ...
  if (!mounted) {
    return (
      <div
        className={`${sizeClasses[size]} ${className} bg-muted rounded animate-pulse`}
      />
    );
  }
  ...
  if (iconConfig && !imageError) {
    ...
    const iconUrl =
      currentTheme === "dark" && iconConfig.dark
        ? iconConfig.dark
        : iconConfig.light;
  ...
  return (
    <div
      className={`${sizeClasses[size]} ${className} bg-muted rounded flex items-center justify-center`}
    >
